...
export async function startSleep(bbyid: BabyId) {
  const current = await getActiveSleep(bbyid);
...
export async function createManualSleep(input: SleepInput) {
  if (
...
export async function getActiveSleep(bbyid: BabyId) {
  const { data, error } = await supabase
    .from("sleep")
    .select("*")
    .eq("bbyid", bbyid)
...
export async function getLastCompletedSleep(bbyid: BabyId) {
  const { data, error } = await supabase
    .from("sleep")
    .select("*")
    .eq("bbyid", bbyid)
