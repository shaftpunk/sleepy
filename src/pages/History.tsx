...
function feedLabel(feed: FeedRecord) {
  if (feed.feedtype === "bottle") {
    return `Bottle${feed.amountml !== null ? ` · ${feed.amountml} ml` : ""}`;
  }

  if (feed.feedtype === "breast") {
    return `Breast${feed.side ? ` · ${feed.side}` : ""}`;
  }
...
export default function History() {
  const bbyid = useAppStore((state) => state.currentBbyId);
  ...
  const [editingFeed, setEditingFeed] =
    useState<FeedRecord | null>(null);

  const [newSleep, setNewSleep] = useState(false);
  const [newFeed, setNewFeed] = useState(false);

  async function load() {
    const [sleepData, feedData] = await Promise.all([
      getRecentSleeps(bbyid, 100),
      getRecentFeeds(bbyid, 100),
    ]);
  ...
  useEffect(() => {
    ...
    const offSleep = subscribeToSleepChanges(bbyid, load);
    const offFeed = subscribeToFeedChanges(bbyid, load);
      ...
      <main className="history-page">
        <header className="page-header">
          ...
          <p className="page-description">
            Correct, add or remove registrations.
          </p>
        </header>
        ...
        <section className="history-section">
          ...
          <div className="history-list">
            {sleeps.map((sleep) => (
              <div className="history-item" key={sleep.id}>
                ...
                >
                  ...
                  <span>{formatDate(sleep.starttime)}</span>
                </div>

                <button
                  className="history-delete"
                  onClick={async () => {
                    if (!window.confirm("Delete this sleep?")) return;
        ...
        <section className="history-section">
          ...
          <div className="history-list">
            {feeds.map((feed) => (
              <div className="history-item" key={feed.id}>
